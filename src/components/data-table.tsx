...
export function DataTableFacetedFilter({
  ...
  onFacetedFilter(options: { value: string; label: string }[]): void;
}) {
  const [selections, setSelections] = React.useState<(typeof options)[number][]>([]);

  const handleSelect = React.useCallback(
    (option?: (typeof options)[number], isSelected?: boolean) => () => {
      // Clear filters and exit early
      if (!option) {
        setSelections([]);
        return onFacetedFilter([]);
      }

      let filteredSelections: typeof selections = [];

      if (isSelected) {
        filteredSelections = selections.filter((s) => s.value != option.value);
      } else {
        filteredSelections = selections.concat(option);
      }

      setSelections(filteredSelections);
      return onFacetedFilter(filteredSelections);
    },
    [selections, onFacetedFilter]
  );
      ...
      <PopoverContent className="w-[200px] p-0" align="start">
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup>
              {options.map((option) => {
                const isSelected = selections.some((s) => s.value === option.value);
                return (
            ...
            {selections.length > 0 && (
              <>
                <CommandSeparator />
                <CommandGroup>
                  <CommandItem onSelect={handleSelect()} className="justify-center text-center">
                    Clear filters
                  </CommandItem>
                </CommandGroup>
              </>
