...
import type { Supplier, SupplierPaginated } from './@types';
...
function SupplierDataTableToolbar({
  table,
  dictionary,
}: {
  ...
  return (
    <div className="flex items-center justify-between">
      {/* prettier-ignore */}
      <DataTableSearchFilter table={table} dictionary={dictionary.supplier.table.toolbar.searchFilter} />
      <DataTableViewOptions table={table} dictionary={dictionary.supplier.table.toolbar.view} />
    </div>
  );
}
...
> &
  Pick<DataTableProps<Supplier>, 'isInitialLoading' | 'isLoading'> &
  Pick<UseDataTableUtilsReturn, 'pagination' | 'sort'> & {
    data?: SupplierPaginated;
  }) {
    ...
    () => [
      {
        ...
        enableSorting: true,
        header: (props) => (
          <DataTableHeadWithSorting title={dictionary.supplier.table.columns.name} {...props} />
        ),
        cell: ({ row }) => (
          <span className="inline-block max-w-[300px] truncate">{row.original?.name ?? ''}</span>
        ),
      },
      {
        ...
        enableSorting: true,
        header: (props) => (
          <DataTableHeadWithSorting title={dictionary.supplier.table.columns.resources} {...props} />
        ),
        cell: ({ row }) => (
          ...
          <div className="flex items-center justify-end space-x-4">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-[160px]">
                ...
                {getPermission('update:supplier').granted && (
                  <DropdownMenuItem onClick={() => table.options.meta?.onRowEdit?.(row.original)}>
                    {dictionary.supplier.table.columns.actions.edit}
                  </DropdownMenuItem>
                )}
                {getPermission('delete:supplier').granted && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onClick={() => table.options.meta?.onRowDelete?.(row.original)}
                    >
                      {dictionary.supplier.table.columns.actions.delete}
                    </DropdownMenuItem>
                  </>
                )}
              </DropdownMenuContent>
  ...
  return (
    <DataTable
      ...
      toolbar={<SupplierDataTableToolbar table={table} dictionary={dictionary} />}
    />
  );
}
