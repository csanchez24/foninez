...
export default function InventoryTransactionFormResources({
  inventoryTransaction,
}: {
  inventoryTransaction?: InventoryTransaction;
}) {
  const dictionary = useStoreContext(
    (state) => state.dictionary.inventoryTransaction.form.resources
  );
  ...
  React.useEffect(() => {
    if (inventoryTransaction?.inventoryTransactionLines)
      replace(
        inventoryTransaction.inventoryTransactionLines
          .filter((l) => l.resource)
          .map((l) => {
            return { qty: l.qty, resource: l.resource! };
          })
      );
  }, [replace, inventoryTransaction]);

  const watchedFields = form.watch('inventoryTransactionLines');
  ...
  return (
    <div className="my-6 space-y-2">
        ...
        <AlertDialog open={opened} onOpenChange={setOpened}>
          ...
          <AlertDialogContent className="sm:max-w-xl" onEscapeKeyDown={(e) => e.preventDefault()}>
            ...
            {entryIndex >= 0 && (
              <>
                <FormField
                  name={`inventoryTransactionLines.${entryIndex}.resource`}
                  control={form.control}
                  render={({ field }) => (
                    <FormItem>
                                ...
                                {resources?.body.data.map((resource) => (
                                  <CommandItem
                                    ...
                                    disabled={watchedFields?.some(
                                      (p) => p.resource.id === resource.id
                                    )}
                                    onSelect={() => {
                                      form.setValue(
                                        `inventoryTransactionLines.${entryIndex}.resource`,
                                        resource
                                      );
                ...
                />
                <FormField
                  name={`inventoryTransactionLines.${entryIndex}.qty`}
                  control={form.control}
                  render={({ field }) => (
                    <FormItem>
      ...
      <Table className="mt-4">
        <TableHeader>
          <TableRow>
            <TableHead>{dictionary.table.columns.resource}</TableHead>
