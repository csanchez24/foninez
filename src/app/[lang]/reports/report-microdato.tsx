...
const FormFields = () => {
  ...
  return (
    <div className="my-6 space-y-2">
      <h2 className="mb-8 text-xl font-bold">{dictionary.tabs.microdato.title}</h2>
      <FormField
        name="programId"
...
export function ReportMicrodato() {
  ...
  const onSubmit = useCallback(async (data: FormValues) => {
    const response = await api.report.microdato.query({
      query: {
        year: data.year,
    ...
    saveAs(blob, 'general_report.xlsx');
  }, []);
