...
import type { Professional, ProfessionalPaginated } from './@types';
...
function ProfessionalDataTableToolbar({
  table,
  dictionary,
}: {
  ...
  return (
    <div className="flex items-center justify-between">
      {/* prettier-ignore */}
      <DataTableSearchFilter table={table} dictionary={dictionary.professional.table.toolbar.searchFilter} />
      <DataTableViewOptions table={table} dictionary={dictionary.professional.table.toolbar.view} />
    </div>
  );
}
...
> &
    ...
    () => [
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.professional.table.columns.idNum}
            {...props}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.professional.table.columns.firstName}
            {...props}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.professional.table.columns.lastName}
            {...props}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.professional.table.columns.email}
            {...props}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.professional.table.columns.isActive}
            {...props}
          />
        ),
        cell: ({ row }) => (
          <span className="inline-block max-w-[300px] truncate">
            {row.original.isActive ? 'Si' : 'No'}
          </span>
        ),
      },
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.professional.table.columns.activities}
            {...props}
          />
        ),
        cell: ({ row }) => (
          <span className="inline-block max-w-[300px] truncate">
            {row.original.planModalityActivitySchoolProfessionals?.length ?? 0}
          </span>
        ),
      },
      {
          ...
          <div className="flex items-center justify-end space-x-4">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-[160px]">
                ...
                {getPermission('update:professional').granted && (
                  <DropdownMenuItem onClick={() => table.options.meta?.onRowEdit?.(row.original)}>
                    {dictionary.professional.table.columns.actions.edit}
                  </DropdownMenuItem>
                )}
                {getPermission('delete:professional').granted && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
  ...
  return (
    <DataTable
      ...
      toolbar={<ProfessionalDataTableToolbar table={table} dictionary={dictionary} />}
    />
  );
}
