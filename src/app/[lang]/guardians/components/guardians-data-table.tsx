...
import type { Guardian, GuardianPaginated } from './@types';
...
function GuardianDataTableToolbar({
  table,
  dictionary,
}: {
  ...
  return (
    <div className="flex items-center justify-between">
      {/* prettier-ignore */}
      <DataTableSearchFilter table={table} dictionary={dictionary.guardian.table.toolbar.searchFilter} />
      <DataTableViewOptions table={table} dictionary={dictionary.guardian.table.toolbar.view} />
    </div>
  );
}
...
> &
    ...
    () => [
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.guardian.table.columns.identification}
            {...props}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        enableSorting: true,
        header: (props) => (
          <DataTableHeadWithSorting title={dictionary.guardian.table.columns.idNum} {...props} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.guardian.table.columns.firstName}
            {...props}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: (props) => (
          <DataTableHeadWithSorting
            title={dictionary.guardian.table.columns.middleName}
            {...props}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        enableSorting: true,
        header: (props) => (
          <DataTableHeadWithSorting title={dictionary.guardian.table.columns.lastName} {...props} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        enableSorting: true,
        header: (props) => (
          <DataTableHeadWithSorting title={dictionary.guardian.table.columns.children} {...props} />
        ),
        cell: ({ row }) => (
          <span className="inline-block max-w-[300px] truncate">
          ...
          <div className="flex items-center justify-end space-x-4">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-[160px]">
                ...
                {getPermission('update:guardian').granted && (
                  <DropdownMenuItem onClick={() => table.options.meta?.onRowEdit?.(row.original)}>
                    {dictionary.guardian.table.columns.actions.edit}
                  </DropdownMenuItem>
                )}
                {getPermission('delete:guardian').granted && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onClick={() => table.options.meta?.onRowDelete?.(row.original)}
                    >
                      {dictionary.guardian.table.columns.actions.delete}
                    </DropdownMenuItem>
                  </>
                )}
              </DropdownMenuContent>
  ...
  return (
    <DataTable
      ...
      toolbar={<GuardianDataTableToolbar table={table} dictionary={dictionary} />}
    />
  );
}
