    ...
    return Object.entries(where).reduce((whereQuery, [k, v]) => {
      ...
      if (
        ...
        if ('like' in v && typeof v.like !== 'undefined') {
          //const t = getTableConfig(table as MySqlTable);
          //const c = t.columns.find((c) => c.name === camelToSnake(k));
          //like(c!, `%${v.like}%`);
          //sql`${table[k as keyof typeof table]} LIKE '%${sql.raw(v.like)}%'`
          //sql.raw(`${snakeToCamel(t.name)}.${c?.name} LIKE '%${v.like}%'`)
          whereQuery.push(
            whereQuery.length === 0
              ? sql`${table[k as keyof typeof table]} LIKE '%${sql.raw(v.like)}%'`
        ...
        // Check if it's 'IN' operator as we need to convert from [?, ?, ?] to (?, ?, ?)
          ...
          whereQuery.push(
            whereQuery.length === 0
              ...
              : sql` AND ${table[k as keyof typeof table]} IN (${sql.join(v.in, sql.raw(','))})`
          );
        }
...
const parseDrizzleFindManyQuery =
  ...
  (q: FindManyQuery) => {
    return {
      ...
      where:
        q.where && Object.keys(q.where).length > 0 ? sql.join(getWhere(table)(q.where)) : undefined,
